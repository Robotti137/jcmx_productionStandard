...
import { postLocationOrder } from "@/api";
let loading = false;
export default {
  updated() {
  ...
  data() {
    return {
      requestUrl,
      locationAddDialogVisible: false,
      locationData: {
        documentNumber: "",
        projectName: "",
        orderDate: "",
  ...
  methods: {
    ...
    addOrder() {
      ...
      if (
        !this.locationData.documentNumber ||
        !this.locationData.projectName ||
        !this.locationData.salesman
      ) {
        this.$notify.error({
          title: "错误",
      ...
      loading = true;
      postLocationOrder(this.locationData).then(data => {
        loading = false;
        let type;
        if (data.status === 1) {
          type = "success";
          this.getOrderList();
          this.locationAddDialogVisible = false;
          this.clearForm();
        } else {
          type = "error";
        }
