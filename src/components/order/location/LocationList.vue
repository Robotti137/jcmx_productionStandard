    ...
    <el-table-column label="操作" fixed="right" width="210" align="center">
      <template slot-scope="scope">
        ...
        <el-button
          ...
          plain
          size="mini"
          @click="setLocationData(scope.row);
...
import { deleteLocationOrder } from "@/api";
let loading = false;
export default {
  created() {
    this.getOrderList();
  ...
  methods: {
    ...mapMutations(["setLocationUpdateDialogVisible", "setLocationData"]),
    ...
    deleteOrder(id) {
      ...
      this.$confirm("此操作将删除此开工单,且后续不可恢复,是否继续?", "提示", {
        ...
        .then(() => {
          loading = true;
          deleteLocationOrder(id).then(data => {
            loading = false;
            let type;
            if (data.status === 1) {
              type = "success";
